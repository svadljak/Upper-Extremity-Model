/*
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * phaseone.c
 *
 * Code generation for function 'phaseone'
 *
 */

/* Include files */
#include "phaseone.h"
#include "Optimization_ThelenMuscle_codegen_data.h"
#include "Optimization_ThelenMuscle_codegen_internal_types.h"
#include "addAineqConstr.h"
#include "addBoundToActiveSetMatrix_.h"
#include "computeFval_ReuseHx.h"
#include "computeGrad_StoreHx.h"
#include "computeQ_.h"
#include "compute_deltax.h"
#include "compute_lambda.h"
#include "deleteColMoveEnd.h"
#include "eml_int_forloop_overflow_check.h"
#include "factorQR.h"
#include "feasibleX0ForWorkingSet.h"
#include "feasibleratiotest.h"
#include "find_neg_lambda.h"
#include "maxConstraintViolation.h"
#include "moveConstraint_.h"
#include "removeAllIneqConstr.h"
#include "rt_nonfinite.h"
#include "setProblemType.h"
#include "squareQ_appendCol.h"
#include "xcopy.h"
#include "xnrm2.h"
#include <emmintrin.h>
#include <string.h>

/* Variable Definitions */
static emlrtRSInfo md_emlrtRSI = {
    1,          /* lineNo */
    "phaseone", /* fcnName */
    "C:\\Program "
    "Files\\MATLAB\\R2024b\\toolbox\\optim\\+optim\\+coder\\+"
    "qpactiveset\\phaseone.p" /* pathName */
};

static emlrtBCInfo gc_emlrtBCI = {
    1,          /* iFirst */
    13,         /* iLast */
    1,          /* lineNo */
    1,          /* colNo */
    "",         /* aName */
    "phaseone", /* fName */
    "C:\\Program "
    "Files\\MATLAB\\R2024b\\toolbox\\optim\\+optim\\+coder\\+"
    "qpactiveset\\phaseone.p", /* pName */
    3                          /* checkKind */
};

static emlrtBCInfo hc_emlrtBCI = {
    1,          /* iFirst */
    23,         /* iLast */
    1,          /* lineNo */
    1,          /* colNo */
    "",         /* aName */
    "phaseone", /* fName */
    "C:\\Program "
    "Files\\MATLAB\\R2024b\\toolbox\\optim\\+optim\\+coder\\+"
    "qpactiveset\\phaseone.p", /* pName */
    0                          /* checkKind */
};

/* Function Definitions */
void phaseone(const emlrtStack *sp, const real_T H[64], const real_T f[13],
              d_struct_T *solution, c_struct_T *memspace,
              f_struct_T *workingset, h_struct_T *qrmanager,
              i_struct_T *cholmanager, j_struct_T *objective,
              const char_T options_SolverName[7],
              const k_struct_T *runTimeOptions)
{
  static const char_T b[7] = {'f', 'm', 'i', 'n', 'c', 'o', 'n'};
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack d_st;
  emlrtStack st;
  f_struct_T r;
  int32_T PROBTYPE_ORIG;
  int32_T TYPE_tmp;
  int32_T activeSetChangeID;
  int32_T globalActiveConstrIdx;
  int32_T i;
  int32_T idxMinLambda;
  int32_T k;
  int32_T nVar_tmp;
  int32_T vectorUB;
  boolean_T subProblemChanged;
  boolean_T updateFval;
  st.prev = sp;
  st.tls = sp->tls;
  b_st.prev = &st;
  b_st.tls = st.tls;
  c_st.prev = &b_st;
  c_st.tls = b_st.tls;
  d_st.prev = &c_st;
  d_st.tls = c_st.tls;
  PROBTYPE_ORIG = workingset->probType;
  nVar_tmp = workingset->nVar;
  i = workingset->nVar + 1;
  if ((i < 1) || (i > 13)) {
    emlrtDynamicBoundsCheckR2012b(i, 1, 13, &gc_emlrtBCI, (emlrtConstCTX)sp);
  }
  solution->xstar[i - 1] = solution->maxConstr + 1.0;
  if (workingset->probType == 3) {
    i = 1;
  } else {
    i = 4;
  }
  st.site = &md_emlrtRSI;
  setProblemType(&st, workingset, i);
  st.site = &md_emlrtRSI;
  removeAllIneqConstr(&st, workingset);
  objective->prev_objtype = objective->objtype;
  objective->prev_nvar = objective->nvar;
  objective->prev_hasLinear = objective->hasLinear;
  objective->objtype = 5;
  objective->nvar = nVar_tmp + 1;
  objective->gammaScalar = 1.0;
  objective->hasLinear = true;
  st.site = &md_emlrtRSI;
  subProblemChanged = true;
  updateFval = true;
  activeSetChangeID = 0;
  i = workingset->nVar;
  globalActiveConstrIdx = 0;
  b_st.site = &nd_emlrtRSI;
  computeGrad_StoreHx(&b_st, objective, H, f, solution->xstar);
  b_st.site = &nd_emlrtRSI;
  solution->fstar = computeFval_ReuseHx(
      &b_st, objective, memspace->workspace_float, f, solution->xstar);
  solution->state = -5;
  b_st.site = &nd_emlrtRSI;
  b_xcopy(&b_st, 23, solution->lambda);
  int32_T exitg1;
  do {
    exitg1 = 0;
    if (solution->state == -5) {
      real_T normDelta;
      boolean_T guard1;
      boolean_T guard2;
      guard1 = false;
      guard2 = false;
      if (subProblemChanged) {
        switch (activeSetChangeID) {
        case 1:
          b_st.site = &nd_emlrtRSI;
          squareQ_appendCol(&b_st, qrmanager, workingset->ATwset,
                            13 * (workingset->nActiveConstr - 1) + 1);
          break;
        case -1:
          b_st.site = &nd_emlrtRSI;
          deleteColMoveEnd(&b_st, qrmanager, globalActiveConstrIdx);
          break;
        default:
          b_st.site = &nd_emlrtRSI;
          factorQR(&b_st, qrmanager, workingset->ATwset, i,
                   workingset->nActiveConstr);
          b_st.site = &nd_emlrtRSI;
          c_st.site = &yb_emlrtRSI;
          computeQ_(&c_st, qrmanager, qrmanager->mrows);
          break;
        }
        b_st.site = &nd_emlrtRSI;
        compute_deltax(
            &b_st, H, solution, memspace, qrmanager, cholmanager, objective,
            memcmp((char_T *)&options_SolverName[0], (char_T *)&b[0], 7) == 0);
        if (solution->state != -5) {
          exitg1 = 1;
        } else {
          b_st.site = &nd_emlrtRSI;
          normDelta = xnrm2(&b_st, i, solution->searchDir);
          if ((normDelta < 1.4901161193847657E-10) ||
              (workingset->nActiveConstr >= i)) {
            guard2 = true;
          } else {
            real_T a;
            b_st.site = &nd_emlrtRSI;
            a = feasibleratiotest(
                &b_st, solution->xstar, solution->searchDir, workingset->nVar,
                workingset->lb, workingset->ub, workingset->indexLB,
                workingset->indexUB, workingset->sizes, workingset->isActiveIdx,
                workingset->isActiveConstr, workingset->nWConstr, true,
                &updateFval, &k, &idxMinLambda);
            if (updateFval) {
              switch (k) {
              case 3:
                r = *workingset;
                b_st.site = &nd_emlrtRSI;
                c_st.site = &nd_emlrtRSI;
                addAineqConstr(&c_st, &r, idxMinLambda);
                break;
              case 4:
                b_st.site = &nd_emlrtRSI;
                c_st.site = &ye_emlrtRSI;
                addBoundToActiveSetMatrix_(&c_st, workingset, idxMinLambda);
                break;
              default:
                b_st.site = &nd_emlrtRSI;
                c_st.site = &bf_emlrtRSI;
                b_addBoundToActiveSetMatrix_(&c_st, workingset, idxMinLambda);
                break;
              }
              activeSetChangeID = 1;
            } else {
              b_st.site = &nd_emlrtRSI;
              if (objective->objtype == 5) {
                c_st.site = &cf_emlrtRSI;
                normDelta = xnrm2(&c_st, objective->nvar, solution->searchDir);
                c_st.site = &cf_emlrtRSI;
                if (normDelta >
                    100.0 * (real_T)objective->nvar * 1.4901161193847656E-8) {
                  solution->state = 3;
                } else {
                  solution->state = 4;
                }
              }
              subProblemChanged = false;
              if (workingset->nActiveConstr == 0) {
                solution->state = 1;
              }
            }
            if (!(a == 0.0)) {
              idxMinLambda = (i / 2) << 1;
              vectorUB = idxMinLambda - 2;
              for (k = 0; k <= vectorUB; k += 2) {
                __m128d r1;
                __m128d r2;
                r1 = _mm_loadu_pd(&solution->searchDir[k]);
                r2 = _mm_loadu_pd(&solution->xstar[k]);
                _mm_storeu_pd(&solution->xstar[k],
                              _mm_add_pd(r2, _mm_mul_pd(_mm_set1_pd(a), r1)));
              }
              for (k = idxMinLambda; k < i; k++) {
                solution->xstar[k] += a * solution->searchDir[k];
              }
            }
            b_st.site = &nd_emlrtRSI;
            computeGrad_StoreHx(&b_st, objective, H, f, solution->xstar);
            updateFval = true;
            guard1 = true;
          }
        }
      } else {
        b_st.site = &nd_emlrtRSI;
        h_xcopy(&b_st, i, solution->searchDir);
        guard2 = true;
      }
      if (guard2) {
        b_st.site = &nd_emlrtRSI;
        compute_lambda(&b_st, memspace->workspace_float, solution, objective,
                       qrmanager);
        if ((solution->state != -7) || (workingset->nActiveConstr > i)) {
          b_st.site = &nd_emlrtRSI;
          idxMinLambda =
              find_neg_lambda(&b_st, solution->lambda,
                              workingset->nActiveConstr, workingset->nWConstr);
          if (idxMinLambda == 0) {
            solution->state = 1;
          } else {
            activeSetChangeID = -1;
            globalActiveConstrIdx = idxMinLambda;
            subProblemChanged = true;
            b_st.site = &nd_emlrtRSI;
            if ((idxMinLambda < 1) || (idxMinLambda > 23)) {
              emlrtDynamicBoundsCheckR2012b(idxMinLambda, 1, 23, &ab_emlrtBCI,
                                            &b_st);
            }
            TYPE_tmp = workingset->Wid[idxMinLambda - 1];
            if ((TYPE_tmp < 1) || (TYPE_tmp > 6)) {
              emlrtDynamicBoundsCheckR2012b(workingset->Wid[idxMinLambda - 1],
                                            1, 6, &t_emlrtBCI, &b_st);
            }
            k = (workingset->isActiveIdx[TYPE_tmp - 1] +
                 workingset->Wlocalidx[idxMinLambda - 1]) -
                1;
            if ((k < 1) || (k > 23)) {
              emlrtDynamicBoundsCheckR2012b(k, 1, 23, &bb_emlrtBCI, &b_st);
            }
            workingset->isActiveConstr[k - 1] = false;
            if (idxMinLambda < workingset->nActiveConstr) {
              c_st.site = &xc_emlrtRSI;
              moveConstraint_(&c_st, workingset, workingset->nActiveConstr,
                              idxMinLambda);
            }
            workingset->nActiveConstr--;
            if ((TYPE_tmp < 1) || (TYPE_tmp > 5)) {
              emlrtDynamicBoundsCheckR2012b(TYPE_tmp, 1, 5, &cb_emlrtBCI,
                                            &b_st);
            }
            workingset->nWConstr[TYPE_tmp - 1]--;
            if (idxMinLambda < workingset->nActiveConstr + 1) {
              k = workingset->nActiveConstr + 1;
              if ((k < 1) || (k > 23)) {
                emlrtDynamicBoundsCheckR2012b(k, 1, 23, &v_emlrtBCI, &st);
              }
              solution->lambda[idxMinLambda - 1] = solution->lambda[k - 1];
            }
            k = workingset->nActiveConstr + 1;
            if ((k < 1) || (k > 23)) {
              emlrtDynamicBoundsCheckR2012b(k, 1, 23, &u_emlrtBCI, &st);
            }
            solution->lambda[k - 1] = 0.0;
          }
        } else {
          idxMinLambda = workingset->nActiveConstr;
          activeSetChangeID = 0;
          globalActiveConstrIdx = workingset->nActiveConstr;
          subProblemChanged = true;
          if ((workingset->nActiveConstr < 1) ||
              (workingset->nActiveConstr > 23)) {
            emlrtDynamicBoundsCheckR2012b(workingset->nActiveConstr, 1, 23,
                                          &v_emlrtBCI, &st);
          }
          b_st.site = &nd_emlrtRSI;
          TYPE_tmp = workingset->nActiveConstr - 1;
          vectorUB = workingset->Wid[TYPE_tmp];
          if ((vectorUB < 1) || (vectorUB > 6)) {
            emlrtDynamicBoundsCheckR2012b(
                workingset->Wid[workingset->nActiveConstr - 1], 1, 6,
                &t_emlrtBCI, &b_st);
          }
          k = (workingset->isActiveIdx[vectorUB - 1] +
               workingset->Wlocalidx[TYPE_tmp]) -
              1;
          if ((k < 1) || (k > 23)) {
            emlrtDynamicBoundsCheckR2012b(k, 1, 23, &bb_emlrtBCI, &b_st);
          }
          workingset->isActiveConstr[k - 1] = false;
          workingset->nActiveConstr = TYPE_tmp;
          if ((vectorUB < 1) || (vectorUB > 5)) {
            emlrtDynamicBoundsCheckR2012b(vectorUB, 1, 5, &cb_emlrtBCI, &b_st);
          }
          workingset->nWConstr[vectorUB - 1]--;
          solution->lambda[idxMinLambda - 1] = 0.0;
        }
        updateFval = false;
        guard1 = true;
      }
      if (guard1) {
        b_st.site = &nd_emlrtRSI;
        solution->iterations++;
        idxMinLambda = objective->nvar;
        if ((solution->iterations >= runTimeOptions->MaxIterations) &&
            ((solution->state != 1) || (objective->objtype == 5))) {
          solution->state = 0;
        }
        if (solution->iterations - solution->iterations / 50 * 50 == 0) {
          c_st.site = &df_emlrtRSI;
          solution->maxConstr =
              c_maxConstraintViolation(&c_st, workingset, solution->xstar);
          normDelta = solution->maxConstr;
          if (objective->objtype == 5) {
            if ((objective->nvar < 1) || (objective->nvar > 13)) {
              emlrtDynamicBoundsCheckR2012b(objective->nvar, 1, 13,
                                            &db_emlrtBCI, &b_st);
            }
            normDelta =
                solution->maxConstr - solution->xstar[objective->nvar - 1];
          }
          if (normDelta > 1.0E-6) {
            boolean_T nonDegenerateWset;
            c_st.site = &df_emlrtRSI;
            f_xcopy(&c_st, objective->nvar, solution->xstar,
                    solution->searchDir);
            c_st.site = &df_emlrtRSI;
            nonDegenerateWset = feasibleX0ForWorkingSet(
                &c_st, memspace->workspace_float, solution->searchDir,
                workingset, qrmanager);
            if ((!nonDegenerateWset) && (solution->state != 0)) {
              solution->state = -2;
            }
            activeSetChangeID = 0;
            c_st.site = &df_emlrtRSI;
            normDelta = c_maxConstraintViolation(&c_st, workingset,
                                                 solution->searchDir);
            if (normDelta < solution->maxConstr) {
              c_st.site = &df_emlrtRSI;
              if (objective->nvar > 2147483646) {
                d_st.site = &h_emlrtRSI;
                check_forloop_overflow_error(&d_st);
              }
              for (vectorUB = 0; vectorUB < idxMinLambda; vectorUB++) {
                if ((vectorUB + 1 < 1) || (vectorUB + 1 > 13)) {
                  emlrtDynamicBoundsCheckR2012b(vectorUB + 1, 1, 13,
                                                &eb_emlrtBCI, &b_st);
                }
                solution->xstar[vectorUB] = solution->searchDir[vectorUB];
              }
            }
          }
        }
        if (updateFval) {
          c_st.site = &df_emlrtRSI;
          solution->fstar = computeFval_ReuseHx(
              &c_st, objective, memspace->workspace_float, f, solution->xstar);
          if ((solution->fstar < 1.0E-6) &&
              ((solution->state != 0) || (objective->objtype != 5))) {
            solution->state = 2;
          }
        }
      }
    } else {
      if (!updateFval) {
        b_st.site = &nd_emlrtRSI;
        solution->fstar = computeFval_ReuseHx(
            &b_st, objective, memspace->workspace_float, f, solution->xstar);
      }
      exitg1 = 1;
    }
  } while (exitg1 == 0);
  st.site = &md_emlrtRSI;
  i = workingset->isActiveIdx[3] + workingset->sizes[3];
  if ((i - 1 < 1) || (i - 1 > 23)) {
    emlrtDynamicBoundsCheckR2012b(i - 1, 1, 23, &fc_emlrtBCI, &st);
  }
  if (workingset->isActiveConstr[i - 2]) {
    boolean_T exitg2;
    st.site = &md_emlrtRSI;
    if (workingset->nActiveConstr > 2147483646) {
      b_st.site = &h_emlrtRSI;
      check_forloop_overflow_error(&b_st);
    }
    vectorUB = 2;
    exitg2 = false;
    while ((!exitg2) && (vectorUB + 1 <= workingset->nActiveConstr)) {
      if ((vectorUB + 1 < 1) || (vectorUB + 1 > 23)) {
        emlrtDynamicBoundsCheckR2012b(vectorUB + 1, 1, 23, &hc_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
      if ((workingset->Wid[vectorUB] == 4) &&
          (workingset->Wlocalidx[vectorUB] == workingset->sizes[3])) {
        st.site = &md_emlrtRSI;
        idxMinLambda = workingset->Wid[vectorUB];
        if ((workingset->Wid[vectorUB] < 1) ||
            (workingset->Wid[vectorUB] > 6)) {
          emlrtDynamicBoundsCheckR2012b(workingset->Wid[vectorUB], 1, 6,
                                        &t_emlrtBCI, &st);
        }
        i = (workingset->isActiveIdx[workingset->Wid[vectorUB] - 1] +
             workingset->Wlocalidx[vectorUB]) -
            1;
        if ((i < 1) || (i > 23)) {
          emlrtDynamicBoundsCheckR2012b(i, 1, 23, &bb_emlrtBCI, &st);
        }
        workingset->isActiveConstr[i - 1] = false;
        if (vectorUB + 1 < workingset->nActiveConstr) {
          b_st.site = &xc_emlrtRSI;
          moveConstraint_(&b_st, workingset, workingset->nActiveConstr,
                          vectorUB + 1);
        }
        workingset->nActiveConstr--;
        if ((idxMinLambda < 1) || (idxMinLambda > 5)) {
          emlrtDynamicBoundsCheckR2012b(idxMinLambda, 1, 5, &cb_emlrtBCI, &st);
        }
        workingset->nWConstr[idxMinLambda - 1]--;
        exitg2 = true;
      } else {
        vectorUB++;
      }
    }
  }
  idxMinLambda = workingset->nActiveConstr;
  while ((idxMinLambda > 2) && (idxMinLambda > nVar_tmp)) {
    st.site = &md_emlrtRSI;
    if (idxMinLambda > 23) {
      emlrtDynamicBoundsCheckR2012b(idxMinLambda, 1, 23, &ab_emlrtBCI, &st);
    }
    TYPE_tmp = workingset->Wid[idxMinLambda - 1];
    if ((TYPE_tmp < 1) || (TYPE_tmp > 6)) {
      emlrtDynamicBoundsCheckR2012b(workingset->Wid[idxMinLambda - 1], 1, 6,
                                    &t_emlrtBCI, &st);
    }
    i = (workingset->isActiveIdx[TYPE_tmp - 1] +
         workingset->Wlocalidx[idxMinLambda - 1]) -
        1;
    if ((i < 1) || (i > 23)) {
      emlrtDynamicBoundsCheckR2012b(i, 1, 23, &bb_emlrtBCI, &st);
    }
    workingset->isActiveConstr[i - 1] = false;
    if (idxMinLambda < workingset->nActiveConstr) {
      b_st.site = &xc_emlrtRSI;
      moveConstraint_(&b_st, workingset, workingset->nActiveConstr,
                      idxMinLambda);
    }
    workingset->nActiveConstr--;
    if ((TYPE_tmp < 1) || (TYPE_tmp > 5)) {
      emlrtDynamicBoundsCheckR2012b(TYPE_tmp, 1, 5, &cb_emlrtBCI, &st);
    }
    workingset->nWConstr[TYPE_tmp - 1]--;
    idxMinLambda--;
  }
  solution->maxConstr = solution->xstar[nVar_tmp];
  st.site = &md_emlrtRSI;
  setProblemType(&st, workingset, PROBTYPE_ORIG);
  objective->objtype = objective->prev_objtype;
  objective->nvar = objective->prev_nvar;
  objective->hasLinear = objective->prev_hasLinear;
}

/* End of code generation (phaseone.c) */
